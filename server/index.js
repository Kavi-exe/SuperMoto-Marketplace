...
app.post('/api/auth/login', loginRateLimiter, async (req, res) => {
  try {
    ...
    if (user.status === 'suspended') {
      db.close();
      return res.status(403).json({ ok: false, error: 'Your account has been suspended. Please contact support.' });
    }
    ...
    const redirect = (role === 'admin' || role === 'super_admin') ? '/admin' : '/dashboard';

    return res.json({ ok: true, user: publicUser(user), accessToken, redirect });
  } catch {
    return res.status(500).json({ ok: false, error: 'Server error' });
  }
});
...
function requireAdmin(req, res, next) {
  // Kept for backward compat — delegates to requireAnyAdmin
  return requireAnyAdmin(req, res, next);
}

// ── Admin Routes ────────────────────────────────────────────────────────────

// Serve admin.html — auth is enforced client-side by admin.js
...
app.get('/api/admin/stats', requireAuth, requireAdmin, async (req, res) => {
  try {
    const db = openDb();
    const stats = await adminGetStats(db);
    db.close();
...
app.get('/api/admin/users', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const db = openDb();
    const result = await adminGetUsers(db, { page, limit, search, role, status });
    db.close();
...
app.patch('/api/admin/users/:id/role', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const db = openDb();
    await adminUpdateUserRole(db, userId, role);
    await adminLogAction(db, { adminId: req.user.id, action: 'update_role', targetType: 'user', targetId: userId, details: `role=${role}` });
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.patch('/api/admin/users/:id/suspend', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const db = openDb();
    await adminSuspendUser(db, userId);
    await adminLogAction(db, { adminId: req.user.id, action: 'suspend_user', targetType: 'user', targetId: userId });
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.patch('/api/admin/users/:id/unsuspend', requireAuth, requireAdmin, async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    const db = openDb();
    await adminUnsuspendUser(db, userId);
    await adminLogAction(db, { adminId: req.user.id, action: 'unsuspend_user', targetType: 'user', targetId: userId });
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.delete('/api/admin/users/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const db = openDb();
    await adminLogAction(db, { adminId: req.user.id, action: 'delete_user', targetType: 'user', targetId: userId });
    await adminDeleteUser(db, userId);
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.get('/api/admin/ads', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const status = String(req.query.status || 'all');
    const db = openDb();
    const result = await adminGetAds(db, { page, limit, search, status });
    db.close();
    return res.json({ ok: true, ...result });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.patch('/api/admin/ads/:id/status', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { status } = req.body || {};
    if (!status) return res.status(400).json({ ok: false, error: 'status is required' });
    const db = openDb();
    await adminSetAdStatus(db, req.params.id, status);
    await adminLogAction(db, { adminId: req.user.id, action: 'set_ad_status', targetType: 'ad', targetId: req.params.id, details: `status=${status}` });
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.patch('/api/admin/ads/:id/featured', requireAuth, requireAdmin, async (req, res) => {
  try {
    const db = openDb();
    await adminToggleAdFeatured(db, req.params.id);
    await adminLogAction(db, { adminId: req.user.id, action: 'toggle_featured', targetType: 'ad', targetId: req.params.id });
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.delete('/api/admin/ads/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    const db = openDb();
    await adminLogAction(db, { adminId: req.user.id, action: 'delete_ad', targetType: 'ad', targetId: req.params.id });
    await adminDeleteAd(db, req.params.id);
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
// Spare Parts
app.get('/api/admin/spare-parts', requireAuth, requireAdmin, async (req, res) => {
  try {
    const page = Math.max(1, parseInt(req.query.page) || 1);
    const limit = Math.min(100, parseInt(req.query.limit) || 15);
    const search = String(req.query.search || '').trim();
    const db = openDb();
    const result = await adminGetSpareParts(db, { page, limit, search });
    db.close();
    return res.json({ ok: true, ...result });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.delete('/api/admin/spare-parts/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    const db = openDb();
    await adminLogAction(db, { adminId: req.user.id, action: 'delete_spare_part', targetType: 'spare_part', targetId: req.params.id });
    await adminDeleteSparePart(db, req.params.id);
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
app.get('/api/admin/logs', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const db = openDb();
    const logs = await adminGetActivityLog(db, limit);
    db.close();
...
// Only super_admin can promote users to admin / demote admins
app.post('/api/super-admin/promote', requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const { userId, role } = req.body || {};
    const allowed = ['user', 'admin'];
    if (!userId || !allowed.includes(role)) {
      return res.status(400).json({ ok: false, error: 'userId and role (user|admin) are required' });
    }
    ...
    if (!target) { db.close(); return res.status(404).json({ ok: false, error: 'User not found' }); }
    // Protect super_admins — cannot demote another super_admin
    if (target.role === 'super_admin') { db.close(); return res.status(403).json({ ok: false, error: 'Cannot modify a super_admin' }); }
    await adminUpdateUserRole(db, targetId, role);
    await adminLogAction(db, { adminId: req.user.id, action: 'super_promote', targetType: 'user', targetId, details: `role=${role}` });
    db.close();
    return res.json({ ok: true });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
...
// ── User Dashboard API ──────────────────────────────────────────────────────
// Get current user's own profile (full detail)
app.get('/api/user/profile', requireAuth, async (req, res) => {
  try {
    ...
    return res.json({ ok: true, user: { ...publicUser(user), status: user.status || 'active', created_at: user.created_at } });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
  }
});
...
app.get('/api/user/listings', requireAuth, async (req, res) => {
  try {
    ...
    const db = openDb();
    const { ads, total } = await adminGetAds(db, { page, limit, status: 'all', search: '' });
    db.close();
    // Filter to only this user's ads
    const mine = ads.filter(a => a.publisherId === req.user.id);
    return res.json({ ok: true, ads: mine, total: mine.length });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
  }
});

// Get current user's own spare parts
app.get('/api/user/spare-parts', requireAuth, async (req, res) => {
  try {
    const db = openDb();
    const { spareParts } = await adminGetSpareParts(db, { page: 1, limit: 100 });
    db.close();
    const mine = spareParts.filter(p => p.publisherId === req.user.id);
    return res.json({ ok: true, spareParts: mine, total: mine.length });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
  }
});
...
app.get('/api/user/stats', requireAuth, async (req, res) => {
  try {
    const db = openDb();
    const { ads } = await adminGetAds(db, { page: 1, limit: 1000, status: 'all' });
    const { spareParts } = await adminGetSpareParts(db, { page: 1, limit: 1000 });
    db.close();
    const myAds     = ads.filter(a => a.publisherId === req.user.id);
    const myParts   = spareParts.filter(p => p.publisherId === req.user.id);
    const activeAds = myAds.filter(a => a.status === 'active').length;
    const pendingAds= myAds.filter(a => a.status === 'pending_payment').length;
    return res.json({ ok: true, stats: {
      totalListings: myAds.length,
      activeListings: activeAds,
      pendingListings: pendingAds,
      totalSpareParts: myParts.length,
    }});
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message || 'Server error' });
  }
...
// Serve user dashboard — auth is enforced client-side by dashboard.js
